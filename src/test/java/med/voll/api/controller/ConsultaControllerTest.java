...
class ConsultaControllerTest {
    ...
    // Mocar esa clase para que no ejecute las consultas a la base de datos
    @MockBean
    private ReservaDeConsultas reservaDeConsultas;

    @Test
    @DisplayName("Debería devolver http 400 cuando la request no tenga datos")
    @WithMockUser // evita tener que tener un token real para los test
    ...
    @Test
    @DisplayName("Debería devolver http 200 cuando la request reciba datos validos")
    @WithMockUser // evita tener que tener un token real para los test
    void reservar_escenario2() throws Exception {

        var fecha = LocalDateTime.now().plusHours(1);
        var especialidad = Especialidad.CARDIOLOGIA;
        var datosDetalleConsulta = new DatosDetalleConsulta(null,1l,5l, fecha);
        ...
        var jsonEsperado = datosDetalleConsultaJson.write(datosDetalleConsulta)
                .getJson();
